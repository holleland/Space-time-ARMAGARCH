    ...
    for(int k=0; k<y.rows(); k++){
      ...
      for(int i = 0; i < alpha.cols(); i++){
        for(int j = 0; j < alpha.rows(); j++){
          if(t-i>=0){
            sigma(k,t+1) +=  alpha(j,i) * W2(k,u,j) * x(u,t-i)*x(u,t-i);
          }
          }
        }
      for(int i = 0; i < beta.cols(); i++){
        for(int j = 0; j < beta.rows(); j++){
          if(t-i>=0){
          sigma(k,t+1) +=  beta(j,i) * W2(k,u,j) * sigma(u,t-i);
          }
        }
      }

      }
      ell(k,t+1) = dnorm(x(k,t+1),Type(0.0),sqrt(sigma(k,t+1)),true);
      f -=  ell(k,t+1);
    }
  }
